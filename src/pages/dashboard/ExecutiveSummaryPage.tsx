...
const badgeStyles: Record<BadgeVariant, string> = {
  ...
  neutral: 'bg-surface-container border-outline-variant/60 text-on-surface-variant',
};
...
const StatCard = ({ icon, label, value, badge, badgeVariant, iconColor = 'text-primary', children }: StatCardProps) => (
  <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 p-md flex flex-col gap-sm shadow-sm">
    <div className="flex items-start justify-between">
      <div className={cn('w-10 h-10 rounded-xl bg-surface-container-low flex items-center justify-center', iconColor)}>
        <span className="material-symbols-outlined text-[22px]">{icon}</span>
      </div>
      <span className={cn('inline-flex items-center px-xs py-[3px] rounded-full text-[11px] font-semibold border', badgeStyles[badgeVariant])}>
        {badge}
      </span>
    </div>
    <div>
      <p className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">{label}</p>
      <p className="font-display-lg text-[28px] font-bold text-on-surface leading-tight mt-xs">{value}</p>
    </div>
    {children}
  </div>
);
...
const CircularProgress = ({ percent, size = 64, stroke = 6 }: { percent: number; size?: number; stroke?: number }) => {
...
const ExecutiveSummaryPage = (): React.ReactElement => {
  return (
    <div className="p-md space-y-md">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="font-headline-lg text-headline-lg text-on-surface">Executive Summary</h1>
      ...
      <div className="grid grid-cols-[1fr_320px] gap-md">
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(135deg, #0d1535 0%, #1a2550 40%, #1e2d60 100%)',
          }}
        >
          {/* Background orb decoration */}
          <div className="absolute -right-8 top-1/2 -translate-y-1/2 w-56 h-56 rounded-full border border-white/5"
          ...
          <div className="absolute -right-4 top-1/2 -translate-y-1/2 w-40 h-40 rounded-full border border-white/10 flex items-center justify-center">
            <div className="w-28 h-28 rounded-full border border-white/10 flex items-center justify-center">
              <div className="w-16 h-16 rounded-full bg-white/8 flex items-center justify-center">
                <span className="material-symbols-outlined text-[32px] text-yellow-300">workspace_premium</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 max-w-[65%]">
            ...
            <p className="font-body-md text-body-md text-white/60 mb-md">
              Complete this single action to massively expand your business support ecosystem.
            </p>

            {/* Stat boxes */}
            <div className="flex gap-sm mb-md">
              <div className="bg-white/8 border border-white/15 rounded-xl px-sm py-xs">
                <p className="font-label-sm text-[10px] text-white/50 uppercase tracking-widest mb-xs">Unlock Potential</p>
                <p className="font-body-md text-[13px] font-bold text-white">5</p>
                <p className="font-body-md text-[12px] text-white/70">Additional Schemes</p>
              </div>
              <div className="bg-white/8 border border-white/15 rounded-xl px-sm py-xs">
                <p className="font-label-sm text-[10px] text-white/50 uppercase tracking-widest mb-xs">Benefit Estimate</p>
                <p className="font-body-md text-[13px] font-bold text-white">₹2.8</p>
        ...
        <div className="space-y-md">
          ...
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 p-md shadow-sm">
            <div className="flex items-center justify-between mb-md">
              <h3 className="font-title-md text-[16px] font-semibold text-on-surface">Compliance Health</h3>
              <div className="relative flex items-center justify-center">
                <CircularProgress percent={82} size={52} stroke={5} />
                <span className="absolute font-label-sm text-[11px] font-bold text-on-surface">82%</span>
              </div>
            </div>

            <div className="space-y-[2px]">
              <ComplianceItem icon="receipt_long" label="GST Registration" status="ACTIVE" />
              <ComplianceItem icon="business_center" label="Udyam Registration" status="PENDING" />
              <ComplianceItem icon="restaurant" label="FSSAI License" status="EXPIRED" />
              <ComplianceItem icon="local_fire_department" label="Fire NOC" status="UPCOMING" />
              <ComplianceItem icon="groups" label="EPF Registration" status="ACTIVE" />
            </div>
          </div>

          {/* Recommended Actions */}
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 p-md shadow-sm">
            <h3 className="font-title-md text-[16px] font-semibold text-on-surface mb-sm">Recommended Actions</h3>
            ...
            <div className="flex items-start gap-sm p-sm rounded-xl bg-surface-container-low border border-outline-variant/40">
              ...
              <div className="min-w-0">
                <p className="font-body-md text-[13px] font-semibold text-on-surface">Apply for Udyam Registration</p>
                <p className="font-label-sm text-label-sm text-on-surface-variant mt-[2px]">Estimated: 15 mins</p>
      ...
      <div className="grid grid-cols-3 gap-md">
        {[
          { name: 'MSME Credit Guarantee', amount: '₹1.2 Lakhs', tag: 'Eligible' },
          ...
          >
            <div>
              <p className="font-body-md text-[13px] font-semibold text-on-surface">{s.name}</p>
              <p className="font-label-sm text-label-sm text-on-surface-variant mt-xs">{s.amount}</p>
            </div>
            <span
              className={cn(
                'px-xs py-[3px] rounded-full text-[11px] font-bold',
                s.tag === 'Eligible' ? 'bg-green-50 text-green-700' : 'bg-orange-50 text-orange-700'
              )}
            >
              {s.tag}
            </span>
