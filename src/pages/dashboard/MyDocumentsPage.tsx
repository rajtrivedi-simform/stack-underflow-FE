...
const STATUS_BADGE: Record<DocStatus, string> = {
  ...
  'Draft': 'bg-surface-container text-on-surface-variant border-outline-variant/60',
  ...
  return (
    <div className="p-md min-h-full relative pb-24">
      ...
      <div className="flex items-center justify-between mb-md">
        <h1 className="font-title-md text-title-md font-bold text-on-surface">My Documents</h1>
      ...
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 shadow-sm overflow-hidden">
        {filtered.map((doc, idx) => (
          ...
          >
            {/* Icon */}
            <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center shrink-0', doc.iconBg)}>
              <span className={cn('material-symbols-outlined text-[22px]', doc.iconColor)}>
                {doc.icon}
              </span>
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-on-surface-variant mt-0.5">Generated {doc.generatedDate}</p>
            </div>
        ...
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12">
            <span className="material-symbols-outlined text-[40px] text-on-surface-variant/30 mb-3">folder_open</span>
            <p className="text-sm text-on-surface-variant">No documents found</p>
          </div>
        )}
      </div>
