    ...
    steps: [
      {
        ...
        description: 'Match your internal sales and purchase books with GSTR-2A/2B data to ensure accuracy.',
      },
      {
        title: 'Prepare GSTR-9 Draft',
        description: 'Consolidate data across all filed monthly returns. Use official utility for verification.',
      },
      {
        title: 'Pay Outstanding Tax',
        description: 'Settle any tax liability discovered during reconciliation using the GST PMT-06 challan.',
      },
      {
        title: 'Submit & Verify',
        description: 'Electronically sign using DSC or EVC and file the return on the GST portal. Download acknowledgement.',
      },
    ],
  },
  {
    ...
    steps: [
      { title: 'Calculate Contributions', description: 'Compute employee (12%) and employer (13.61%) contributions for all eligible employees.' },
      { title: 'Generate ECR', description: 'Create Electronic Challan cum Return on the EPFO unified portal.' },
      { title: 'Pay Challan', description: 'Pay via net banking using the generated ECR challan before the 15th.' },
      { title: 'Download Receipt', description: 'Save the payment receipt and update payroll records.' },
    ],
  },
  {
    ...
    steps: [
      { title: 'Check Applicability', description: 'Verify if your state levies PT and the applicable threshold.' },
      { title: 'Apply for PTEC', description: 'Apply for Professional Tax Enrollment Certificate on the state commercial tax portal.' },
      { title: 'Apply for PTRC', description: 'Apply for Professional Tax Registration Certificate if you have employees.' },
      { title: 'File Returns', description: 'File monthly/annual PT returns and deposit tax as applicable.' },
    ],
  },
  {
    ...
    steps: [
      { title: 'Gather Documents', description: 'Collect existing license, property tax receipt, and NOC from landlord if applicable.' },
      { title: 'Apply Online', description: 'Visit the municipal corporation portal and apply for renewal with updated details.' },
      { title: 'Pay Fees', description: 'Pay the prescribed renewal fees online.' },
      { title: 'Collect License', description: 'Download the digital license or collect the physical copy from the municipal office.' },
    ],
  },
  {
    ...
    missingDocs: ['FSSAI License Copy', 'Product Label Approval', 'Hygiene Training Records'],
    steps: [
      { title: 'Review FSSAI License', description: 'Ensure your FSSAI license is valid and prominently displayed at the premises.' },
      { title: 'Update Food Safety Plan', description: 'Review and update your HACCP or food safety management plan.' },
      { title: 'Check Product Labels', description: 'Verify all product labels comply with FSSAI labelling regulations.' },
      { title: 'Prepare Documents', description: 'Organise all records for auditor inspection.' },
    ],
  },
  {
  ...
  >
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2 mb-0.5">
        ...
        <span className={cn('text-[10px] font-bold px-1.5 py-0.5 rounded', item.badgeColor)}>
          {item.badge}
        </span>
      </div>
...
const CompliancePage = (): React.ReactElement => {
  ...
  const criticalItems = COMPLIANCE_ITEMS.filter(i => i.severity === 'critical');
  const attentionItems = COMPLIANCE_ITEMS.filter(i => i.severity === 'attention');
  const compliantItems = COMPLIANCE_ITEMS.filter(i => i.severity === 'compliant');
  ...
  return (
    <div className="p-md relative min-h-full pb-24">
      ...
      <div className="flex items-center justify-between mb-md">
        <div>
          <h1 className="font-title-md text-title-md font-bold text-on-surface">Compliance Check</h1>
          <p className="font-label-sm text-label-sm text-on-surface-variant mt-0.5">Last checked: Jun 13, 2026</p>
        </div>
        <div className="flex items-center gap-sm">
          <button className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-surface-container-low transition-colors">
            <span className="material-symbols-outlined text-[20px] text-on-surface-variant">sync</span>
          </button>
          <button className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-surface-container-low transition-colors">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex items-center gap-3 mt-4 mb-4">
            <div className="flex items-center gap-1.5 bg-green-50 border border-green-200 rounded-full px-3 py-1">
              ...
              <span className="text-xs font-semibold text-green-700">{compliantItems.length + 9} Compliant</span>
            </div>
            <div className="flex items-center gap-1.5 bg-orange-50 border border-orange-200 rounded-full px-3 py-1">
              <span className="w-2 h-2 rounded-full bg-orange-500" />
              <span className="text-xs font-semibold text-orange-700">{attentionItems.length} Needs Attention</span>
            </div>
            <div className="flex items-center gap-1.5 bg-red-50 border border-red-200 rounded-full px-3 py-1">
              <span className="w-2 h-2 rounded-full bg-red-500" />
              <span className="text-xs font-semibold text-red-700">{criticalItems.length} Critical Issues</span>
            </div>
          </div>
      ...
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 shadow-sm overflow-hidden mb-md">
        <div className="flex items-center justify-between px-4 py-3 border-b border-outline-variant/30">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-red-500" />
            <span className="text-sm font-bold text-on-surface">Critical Issues</span>
          </div>
        ...
        <div className="divide-y divide-outline-variant/20">
          {criticalItems.map(item => (
            <CriticalRow key={item.id} item={item} onClick={() => goToDetail(item.id)} />
          ))}
        </div>
      </div>
      ...
      <div className="mb-md">
        <div className="flex items-center gap-2 mb-3">
          <span className="w-2 h-2 rounded-full bg-orange-500" />
          <span className="text-sm font-bold text-on-surface">Needs Attention</span>
        </div>
        <div className="flex gap-3">
          {attentionItems.map(item => (
            <AttentionCard key={item.id} item={item} onClick={() => goToDetail(item.id)} />
          ))}
        </div>
      </div>
      ...
      <div className="fixed bottom-6 right-6 z-30">
        <button
          ...
          disabled={isRunning}
          className="flex items-center gap-2 h-12 px-5 bg-orange-500 hover:bg-orange-600 active:scale-[0.97] text-white font-semibold text-sm rounded-full shadow-lg shadow-orange-500/30 transition-all disabled:opacity-80"
        >
          <span className={cn(
