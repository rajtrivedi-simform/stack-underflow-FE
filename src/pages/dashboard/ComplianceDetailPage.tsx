    ...
    <div className="p-md pb-28 relative min-h-full">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-title-md text-[17px] font-bold text-on-surface">{item.title}</h1>
        </div>
        <div className="flex items-center gap-2">
          {/* Health Score chip */}
          <div className="flex items-center gap-1.5 px-3 py-1 rounded-full border border-green-200 bg-green-50">
            <span className="w-2 h-2 rounded-full bg-green-500" />
            <span className="text-xs font-semibold text-green-700">Health Score: 82</span>
          </div>
      ...
      {/* Status Banner */}
      <div className={cn('rounded-xl border p-4 flex items-center justify-between mb-5', statusBg)}>
        <div className="flex items-center gap-3">
          <span className={cn('text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider', statusBadgeBg)}>
            {isCritical ? 'Non-Compliant' : isAttention ? 'Needs Attention' : 'Compliant'}
          </span>
      ...
      <div className="flex gap-5">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-sm font-bold text-on-surface">Why This Matters</h2>
            </div>
          ...
          {item.missingDocs.length > 0 && (
            <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-sm font-bold text-on-surface">What's Missing</h2>
              </div>
              <div className="space-y-2">
                {item.missingDocs.map(doc => (
                  <div key={doc} className="flex items-center gap-2 py-2 border-b border-outline-variant/20 last:border-0">
            ...
            <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-bold text-on-surface">How to Fix — Step by Step</h2>
                </div>
                <div className="flex items-center gap-3">
              ...
              <div className="space-y-3">
                {item.steps.map((step, index) => (
                  <div key={step.title} className="flex gap-3">
                    ...
                    {/* Content */}
                    <div className={cn('flex-1 pb-3', index < item.steps.length - 1 && 'border-b border-outline-variant/20')}>
        ...
        <div className="w-64 shrink-0 space-y-3">
          ...
          {(item.penalty || item.risk) && (
            <div className="rounded-2xl border border-red-200 bg-red-50 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-bold text-red-700 uppercase tracking-wider">Penalty for Non-Compliance</p>
              </div>
              ...
              {item.penalty && (
                <p className="text-xs text-red-600/80 leading-relaxed">
                  Delay beyond 5 months triggers legal notice and suspension of e-way bill generation.
                </p>
              )}
            </div>
          )}
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="material-symbols-outlined text-primary text-[18px]">assistant</span>
              <p className="text-xs font-bold text-primary uppercase tracking-wider">AI Advisory Insight</p>
            </div>
            <p className="text-xs text-on-surface-variant leading-relaxed">
              "Your current sales data suggests high transaction volume. Immediate filing will prevent an estimated compounding interest of ₹14,000 by end of quarter."
            </p>
          </div>

          {/* Progress tracker */}
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/40 p-4 shadow-sm">
            <p className="text-xs font-bold text-on-surface mb-3">Resolution Progress</p>
            <div className="space-y-2.5">
              {item.steps.map((step, i) => (
                <div key={step.title} className="flex items-center gap-2">
                  <div className={cn(
                    'w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0',
                    i === 0 ? 'border-primary bg-primary' : 'border-outline-variant'
                  )}>
                    {i === 0 && <span className="material-symbols-outlined text-white text-[10px]">check</span>}
                  </div>
                  <p className={cn('text-xs', i === 0 ? 'text-on-surface font-medium' : 'text-on-surface-variant/60')}>
                    {step.title}
                  </p>
                </div>
              ))}
